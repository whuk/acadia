...
- [x] 10.2 서비스 연결 실패 시 빈 목록을 반환한다
- [x] 10.3 동적으로 가져온 그룹이 Swagger URL에 서비스/그룹 형식으로 추가된다
- [x] 10.4 동적 페칭 실패 시 정적 swaggerGroups 설정을 폴백으로 사용한다
- [x] 10.5 그룹이 없는 서비스는 기존처럼 서비스 단위로 URL이 생성된다
- [x] 10.6 /v3/api-docs/{service}/{group} 요청이 백엔드의 /v3/api-docs/{group}으로 라우팅된다
...
- [x] 11.4 바디 크기가 설정된 최대 크기를 초과하면 잘라서 기록된다 (max-body-size)
- [x] 11.5 민감한 필드는 마스킹 처리된다 (password, token 등)
- [x] 11.6 multipart/form-data 요청은 바디 로깅에서 제외된다
- [x] 11.7 응답 헤더가 로그에 기록된다

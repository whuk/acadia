...
- `X-B3-TraceId`는 인입 헤더가 유효하면 보존하여 트레이스 연속성을 유지하고, 없거나 형식이 부적합하면 생성한다.
- `X-B3-SpanId`는 hop마다 새로 생성하여 다운스트림으로 전달한다. 인입 SpanId가 유효하면 `X-B3-ParentSpanId`로 전달하여 게이트웨이 span을 부모와 연결한다.
- 인입 트레이스 헤더는 신뢰 전 형식 검증한다: TraceId는 16 또는 32자리 hex, SpanId는 16자리 hex. 부적합하면 보존하지 않고 생성으로 폴백한다(로그 인젝션·트레이스 오염 방지).
...
- 인입 `X-B3-TraceId`를 무조건 덮어써 트레이스 연속성을 끊기.
- 인입 트레이스 헤더를 형식 검증 없이 신뢰하여 다운스트림에 전달.
